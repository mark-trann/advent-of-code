...
int join(point p1, point p2) {
    int originalX1 = get<0>(p1);
    int originalX2 = get<0>(p2);
    p1 = root(p1);
    p2 = root(p2);
    auto [x1, y1, z1] = p1;
    auto [x2, y2, z2] = p2;
    if (p1 == p2) return originalX1 * originalX2;
...
void solution() {
    ...
    init();
    int res = 0;
    for (auto pt : pairPoints) {
        auto [d, p1, p2] = pt;
        res = join(p1, p2);
        if (relevantCoords.size() == 1) break;
    }
    cout << res << endl;
}
