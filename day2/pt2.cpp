...
bool isInvalid(int num) {
    ...
    for (int l = 1; l <= numStr.length() / 2; l++) {
        if (numStr.length() % l != 0) continue;
        string subStr = numStr.substr(0, l);
        bool flag = false;
        for (int i = 0; i < numStr.length(); i += l) {
            if (subStr != numStr.substr(i, l)) {
                flag = true;
                break;
            }
        }
...
void solution() {
    char d1, d2;
    int start, end, res = 0;
    while (cin >> start >> d1 >> end >> d2) {
        res += rangeSum(start, end);
    }
    cin >> start >> d1 >> end;
    res += rangeSum(start, end);
    cout << res << endl;
