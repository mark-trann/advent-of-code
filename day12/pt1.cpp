...
int countNums(string line) {
    int res = 0;
    auto spacePos = line.find(' ');
    while (spacePos != string::npos) {
        cout << line << endl;
        res += stoll(line.substr(0, spacePos));
...
bool isValid(string line) {
    ...
    int num1 = stoll(line.substr(0, xPos));
    auto colonPos = line.find(':');
    int num2 = stoll(line.substr(xPos + 1, colonPos - xPos - 1));
    return countNums(line.substr(colonPos + 2)) * 9 <= num1 * num2;
}
