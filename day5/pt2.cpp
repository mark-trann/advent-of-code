...
void solution() {
    ...
    while (getline(cin, line)) {
        ...
        int startTime = stoll(line.substr(0, dashPos));
        int endTime = stoll(line.substr(dashPos + 1));
    ...
    for (auto range : ranges) {
        ...
        int endTime = range[1];
        bool append = true;
        for (vector<int>& finalRange : final) {
            int start = finalRange[0];
            if (endTime >= start) {
                finalRange[0] = min(startTime, finalRange[0]);
                finalRange[1] = max(endTime, finalRange[1]);
                append = false;
                break;
            }
        }
        if (append) final.push_back({ startTime, endTime });
    }
