...
void solution() {
    ...
    int res = 0;
    int index = operation.size() - 1;
    int tmp = operation[index] == '*';
    for (int j = 0; j < mathLines[0].length(); j++) {
        string numStr = "";
        bool isDelim = true;
        for (int i = 0; i < mathLines.size(); i++) {
            if (mathLines[i][j] != ' ') {
                numStr += mathLines[i][j];
                isDelim = false;
            }
        }
        // cout << boolalpha << numStr << "," << isDelim << ", " << res << endl;
        if (isDelim) {
            index--;
            res += tmp;
            tmp = operation[index] == '*';
        } else {
            tmp = (operation[index] == '*') ? tmp * stoll(numStr) : tmp + stoll(numStr);
        }
    }
    res += tmp;
    cout << res << endl;
